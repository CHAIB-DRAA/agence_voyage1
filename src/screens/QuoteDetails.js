...
import { 
  ...
  ActivityIndicator, 
  Platform,
  Image // <--- IMPORT CRUCIAL POUR L'APERÇU
} from 'react-native';
...
export default function QuoteDetails({ route, navigation }) {
  ...
  const totals = useMemo(() => {
    if (!q) return { hotelTotal: 0, fixedTotal: 0, grandTotal: 0, numPeople: 1, flight: 0, transport: 0, visa: 0 };
    ...
    return { hotelTotal, fixedTotal, grandTotal, numPeople, flight, transport, visa };
  }, [q]);
  ...
  const shareToWhatsApp = () => {
    const text = [
      ...
      `🚌 النقل الداخلي: ${q.transportMakkahMedina || 'حافلة'}`,
      `📄 التأشيرة: متضمنة`,
      `----------------`,
    ...
    try {
      ...
      const costSummaryBlock = !isClient ? `
        ...
        <div class="cost-summary">
          ...
          <div class="cost-row" style="border-top:2px solid #eee;font-weight:bold;"><span>Total</span><span style="color:#F3C764">${totals.grandTotal} DA</span></div>
        </div>
      ` : '';
      ...
      const htmlContent = `
        ...
        <head>
          ...
          <style>
            ...
            .client-meta { font-size: 14px; color: #666; margin-top: 5px; }
            ...
            .total-box { margin-top: 40px; text-align: left; background: #F3C764; color: #050B14; padding: 20px; border-radius: 8px; display: inline-block; float: left; min-width: 250px; }
            .total-val { font-size: 28px; font-weight: 900; margin-top: 5px; }
            ...
            .cost-summary { width: 60%; float: left; margin-top: 10px; font-size: 14px; border: 1px solid #eee; padding: 15px; border-radius: 8px; background: #fffbe6; }
        ...
        <body>
          ...
          <div class="section-title">الخيارات</div>
          <table><thead><tr><th>نوع الغرفة</th><th style="text-align:center">العدد</th>${!isClient?'<th>الكلفة</th>':''}</tr></thead><tbody>${tableRows}</tbody></table>
          ${costSummaryBlock}
          <div class="total-box">
            <div class="total-lbl">المبلغ الإجمالي للدفع</div>
            <div class="total-val">${totals.grandTotal} DA</div>
          </div>
          <div class="footer"><p>شكراً لثقتكم بخدماتنا.</p></div>
        </body>
        </html>
      `;
  ...
  // Helpers UI
  const InfoRow = ({ label, value }) => (<View style={styles.infoRow}><Text style={styles.label}>{label}</Text><Text style={styles.value}>{value || '-'}</Text></View>);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.clientCard}>
           <View style={styles.rowReverse}>
              ...
              <View style={styles.totalBadge}><Text style={styles.totalLabel}>الإجمالي</Text><Text style={styles.totalValue}>{totals.grandTotal} DA</Text></View>
           </View>
           {/* --- AJOUT DE L'APERÇU PASSEPORT ICI --- */}
           {q.passportImage && (
        ...
        <View style={styles.card}>
          <View style={styles.cardHeaderRow}><Feather name="dollar-sign" size={20} color="#F3C764" /><Text style={styles.cardTitle}>ملخص التكاليف</Text></View>
          <View style={styles.divider} />
          <InfoRow label="إجمالي الفنادق" value={`${totals.hotelTotal} DA`} />
          <InfoRow label={`طيران (${totals.numPeople} pax)`} value={`${totals.flight * totals.numPeople} DA`} />
          <InfoRow label={`نقل (${totals.numPeople} pax)`} value={`${totals.transport * totals.numPeople} DA`} />
          <InfoRow label={`تأشيرة (${totals.numPeople} pax)`} value={`${totals.visa * totals.numPeople} DA`} />
          <View style={[styles.divider, {backgroundColor:'#F3C764'}]} />
          <InfoRow label="المجموع الكلي" value={`${totals.grandTotal} DA`} />
        </View>
